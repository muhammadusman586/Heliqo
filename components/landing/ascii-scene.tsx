...
export function AsciiScene() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Sit right of the hero headline on desktop, centered behind it on mobile
      const centerX = width * (width >= 1024 ? 0.72 : 0.5);
      ...
      // Rendu ASCII
      const charSize = Math.max(10, Math.min(width, height) * 0.014);
